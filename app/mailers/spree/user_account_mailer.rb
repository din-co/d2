module Spree
  class UserAccountMailer < BaseMailer
    ...
    def credit_card_changed_email(credit_card)
      ...
      mail(to: @credit_card.user.email, subject: build_subject(@title), from: from_address(@store))
    end
    ...
    def meal_preferences_changed_email(meal_preference)
      ...
      mail(to: @meal_preference.user.email, subject: build_subject(@title), from: from_address(@store))
    end
    ...
    def delivery_address_changed_email(address, user)
      ...
      mail(to: user.email, subject: build_subject(@title), from: from_address(@store))
    end
    ...
    def meal_subscription_changed_email(meal_subscription)
      ...
      mail(to: @meal_subscription.user.email, subject: build_subject(@title), from: from_address(@store))
    end
