IntercomRails.config do |config|
  # == Intercom app_id
  #
  config.app_id = ENV["INTERCOM_APP_ID"] || "qdb0x0r9"
  ...
  #
  config.user.current = Proc.new { spree_current_user }
  ...
  #
  config.user.exclude_if = Proc.new { |user| user.deleted? || user.email == 'admin@din.co' }
